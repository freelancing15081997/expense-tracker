...
async function copyLegacyErp(sql) {
  try {
...
async function erpGet(path) {
  ...
  if (legacy) await erpSet(path, legacy).catch(() => void 0);
  return legacy;
}
async function erpSet(path, data, insertOnly = false) {
  const parsed = parseErpPath(path);
...
async function erpDel(path) {
  ...
  const sql = await getLedgerSql();
  const stamp = JSON.stringify({ deleted: true, deletedAt: (/* @__PURE__ */ new Date()).toISOString() });
  await sql`
    UPDATE erp_records
    SET deleted = TRUE,
...
async function erpList(prefix) {
  ...
  for (const row of legacy) {
    ...
    await erpSet(`${base}${rest}`, data).catch(() => void 0);
  }
  return out;
}
/**
...
async function erpLoadWorkspace(ws) {
  const id = text(ws);
  const sql = await getLedgerSql();
  const tenantRows = asRows(await sql`SELECT data FROM erp_workspaces WHERE id = ${id} LIMIT 1`);
  const recs = asRows(
    await sql`
  ...
  for (const row of legacy) {
    ...
    docs[rel] = data;
    await erpSet(`${prefix}${rel}`, data).catch(() => void 0);
  }
  return { tenant, docs };
}
...
async function ledgerFindUidByEmail(email) {
...
async function ledgerForgetBook(bookId, uid) {
  const id = text(bookId);
  const userId = text(uid);
  if (!id || !userId) {
    const err = new Error("Missing ledger");
    err.status = 400;
    throw err;
  }
  if (await ledgerMember(id, userId)) {
    const err = new Error("Leave this book first, then you can remove it from your list.");
    err.status = 400;
    throw err;
  }
  const sql = await getLedgerSql();

...
export function MoneySheet({
  ...
  return (
    <div className="fixed inset-0 z-[140] flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full sm:max-w-md max-h-[92dvh] overflow-hidden rounded-t-[28px] sm:rounded-[28px] bg-white shadow-[0_-24px_80px_-20px_rgba(30,45,120,0.45)] animate-money-rise">
        ...
        <div className="flex items-start justify-between gap-3 px-5 pt-4 pb-3 border-b border-slate-100">
          <div className="min-w-0">
            <h2 className="font-display text-[20px] font-semibold tracking-[-0.03em] text-[#0B0F1F]">{title}</h2>
...
export function ReceiptFlowProgress({ state }: { state: ReceiptFlowState | string }) {
  ...
  return (
    <div className={cn('rounded-2xl border px-4 py-3.5', failed ? 'border-rose-200 bg-rose-50' : done ? 'border-indigo-200 bg-indigo-50/70' : 'border-slate-200 bg-gradient-to-br from-white to-slate-50')}>
      <div className="flex items-start gap-3">
        <span className={cn('w-10 h-10 rounded-2xl inline-flex items-center justify-center shrink-0', failed ? 'bg-rose-100 text-rose-700' : done ? 'bg-indigo-100 text-indigo-800' : 'bg-[#3654FF] text-white')}>
          {failed ? <AlertCircle className="w-5 h-5" /> : done ? <Check className="w-5 h-5" /> : <Loader2 className="w-5 h-5 animate-spin" />}
        </span>
        <div className="min-w-0">
          <p className="text-[15px] font-semibold text-[#0B0F1F] tracking-[-0.02em]">{copy.title}</p>
  ...
  if (!contexts.length) {
    return (
      <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-[13px] text-amber-950">
        No authorized Money book yet. Create one first â€” your receipt stays safe and nothing was posted.
      </div>
    );
  }
  return (
        ...
        return (
          ...
          >
            ...
            <span className="sr-book-copy min-w-0">
              ...
              <span className="sr-book-reason">{ctx.reason}{ctx.currency ? ` Â· ${ctx.currency}` : ''}</span>
            </span>
...
export function ExpenseSuccessCard({
  ...
  return (
    <div className="rounded-[24px] border border-indigo-100 bg-gradient-to-b from-indigo-50/80 to-white p-5 text-center shadow-[0_18px_40px_-24px_rgba(14,163,150,0.55)]">
      <div className="mx-auto w-12 h-12 rounded-full bg-[#3654FF] text-white inline-flex items-center justify-center shadow-lg shadow-indigo-500/30">
        <Check className="w-6 h-6" strokeWidth={2.5} />
      </div>
      <p className="mt-3 text-[13px] font-semibold uppercase tracking-[0.14em] text-[#2440DB]">
        {count && count > 1 ? `${count} entries added` : 'Expense added'}
      </p>
      <p className="mt-1 font-display text-[34px] font-semibold tracking-[-0.04em] text-[#0B0F1F]">{formatPaise(amountPaise, symbol)}</p>
      <div className="mt-4 space-y-1.5 text-[13px] text-slate-600">
        {merchant ? <p className="font-medium text-[#0B0F1F]">{merchant}</p> : null}
        {category ? <p>{category}</p> : null}
        {bookName ? <p className="text-slate-500">Money Â· {bookName}</p> : null}
        {receiptAttached ? (
          <p className="inline-flex items-center gap-1.5 text-indigo-800 justify-center">
      ...
      <div className="mt-5 flex flex-col gap-2">
        {onSplit ? (
          <button
            type="button"
            onClick={onSplit}
            className="w-full h-12 rounded-2xl bg-gradient-to-b from-[#16325a] to-[#0B0F1F] text-white text-[13px] font-semibold shadow-[0_14px_28px_-14px_rgba(30,45,120,0.9),inset_0_1px_0_rgba(255,255,255,0.18)]"
          >
            Split with team
          </button>
        ) : null}
        <div className="flex gap-2">
          {onView ? (
            <button type="button" onClick={onView} className="flex-1 h-11 rounded-xl border border-slate-200 bg-white text-[13px] font-semibold text-[#0B0F1F]">
              View
            </button>
          ) : null}
          <button type="button" onClick={onDone} className="flex-1 h-11 rounded-xl border border-slate-200 bg-white text-[13px] font-semibold text-[#0B0F1F]">
            Done
          </button>
        </div>
      </div>
...
export function ExpenseErrorCard({
  ...
  return (
    <div className="rounded-[24px] border border-rose-100 bg-rose-50/60 p-5 text-center">
      ...
      <p className="mt-3 font-display text-[18px] font-semibold text-[#0B0F1F]">We couldnâ€™t finish this receipt.</p>
      <p className="mt-1 text-[13px] text-slate-600 leading-relaxed">Your receipt is safe. Nothing was added twice.</p>
      <div className="mt-5 flex gap-2">
        {onRetry ? (
          <button type="button" onClick={onRetry} className="flex-1 h-11 rounded-xl bg-[#3654FF] text-white text-[13px] font-semibold inline-flex items-center justify-center gap-2">
            <RefreshCw className="w-4 h-4" /> Try again
          </button>
        ) : null}
        {onViewReceipt ? (
          <button type="button" onClick={onViewReceipt} className="flex-1 h-11 rounded-xl border border-slate-200 bg-white text-[13px] font-semibold text-[#0B0F1F]">
            View receipt
          </button>
        ) : null}
      </div>
    ...
    <ol className="relative space-y-0 pl-1">
      {events.map((ev, i) => (
        <li key={ev.id} className="relative flex gap-3 pb-4 last:pb-0">
          ...
          <span className="relative z-[1] mt-1 w-[18px] h-[18px] rounded-full border-2 border-indigo-400 bg-white shrink-0" />
          <div className="min-w-0 pt-0.5">
            <p className="text-[13px] font-semibold text-[#0B0F1F] capitalize">{ev.title}</p>
...
export function SoftToast({ message }: { message: string }) {
  return (
    <div className="pointer-events-none fixed left-1/2 bottom-[calc(5.75rem+env(safe-area-inset-bottom))] z-[160] -translate-x-1/2 px-4 w-full max-w-sm">
      <div className="rounded-2xl bg-[#3654FF] text-white text-[13px] font-semibold px-4 py-3 shadow-2xl shadow-slate-900/35 border border-white/10 text-center tracking-[-0.01em]">
        {message}
      </div>
    </div>
  );

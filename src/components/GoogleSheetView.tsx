...
export const GoogleSheetView: React.FC<Props> = ({
  ...
  return (
    <div id="google-sheet-container" className="bg-white rounded-xl shadow-md border border-slate-200 overflow-hidden flex flex-col">
      ...
      <div className="bg-slate-50 border-b border-slate-200 px-4 py-2.5 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {/* Standard Spreadsheet Menu Options */}
            <div className="flex items-center gap-2.5 text-[11px] text-slate-600 font-medium mt-0.5">
              ...
              <span className="text-emerald-700 font-semibold flex items-center gap-1">
                <Check className="w-3 h-3" /> All changes broadcasted to emails
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="bg-slate-100/80 border-b border-slate-200 px-3 py-1.5 flex flex-wrap items-center justify-between gap-2 text-xs">
        <div className="flex items-center gap-1 text-slate-700">
          <button className="p-1 hover:bg-slate-200 rounded text-slate-600" title="Undo"><Undo2 className="w-3.5 h-3.5" /></button>
          <button className="p-1 hover:bg-slate-200 rounded text-slate-600" title="Redo"><Redo2 className="w-3.5 h-3.5" /></button>
          <button onClick={() => window.print()} className="p-1 hover:bg-slate-200 rounded text-slate-600" title="Print Sheet"><Printer className="w-3.5 h-3.5" /></button>
          <div className="w-px h-4 bg-slate-300 mx-1" />
          <button className="p-1 hover:bg-slate-200 rounded font-bold text-slate-700" title="Format Currency"><DollarSign className="w-3.5 h-3.5" /></button>
          <button className="p-1 hover:bg-slate-200 rounded font-bold text-slate-700" title="Format Percent"><Percent className="w-3.5 h-3.5" /></button>
          <div className="w-px h-4 bg-slate-300 mx-1" />
          <button 
            onClick={() => setIsBold(!isBold)} 
            className={`p-1 rounded font-bold ${isBold ? 'bg-slate-300 text-slate-900' : 'hover:bg-slate-200 text-slate-700'}`}
            title="Bold"
          >
          ...
          <button 
            ...
            className={`p-1 rounded ${isItalic ? 'bg-slate-300 text-slate-900' : 'hover:bg-slate-200 text-slate-700'}`}
            title="Italic"
          >
            <Italic className="w-3.5 h-3.5" />
          </button>
          <div className="w-px h-4 bg-slate-300 mx-1" />
          <div className="flex items-center gap-1 bg-white px-2 py-0.5 rounded border border-slate-300 text-[11px] font-mono text-emerald-800">
            <span>fx:</span>
            <span className="font-semibold">=SUM(C2:C{expenses.length + 1})</span>
          </div>
        </div>
                  ...
                  <td
                    ...
                    className={`px-3 py-2 border-r border-slate-200 font-mono font-bold text-right cursor-cell relative ${
                      selectedCell?.rowId === exp.id && selectedCell?.field === 'amount'
                        ? 'ring-2 ring-emerald-500 ring-inset bg-emerald-50/40'
                        : ''
                    } ${isBold ? 'font-extrabold' : ''} ${isItalic ? 'italic' : ''}`}
                  >

...
export default function AppLoader({
  ...
  const body = (
    <div className="flex flex-col items-center justify-center gap-5 text-center" role="status" aria-live="polite">
      <div className="relative">
        <span className="absolute inset-[-18px] rounded-[28px] bg-[radial-gradient(circle,rgba(18,184,168,0.28),transparent_68%)]" aria-hidden />
        <BrandLogo size="md" className="relative shadow-none" />
      </div>
      <div className="space-y-1.5 max-w-xs">
        <p className="font-display text-[18px] font-semibold tracking-tight text-[#0B1F3A]">{heading}</p>
  ...
  if (overlay) {
    return (
      <div className="fixed inset-0 z-[9999] bg-[#0B1F3A] flex items-center justify-center">

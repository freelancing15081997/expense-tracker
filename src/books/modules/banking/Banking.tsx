...
type Tab = 'recon' | 'move' | 'import';

export default function Banking() {
  const books = useBooks();
  const { postingAccounts, bankTxns, currency, can, transfer, createBankTxn, importBankTxns, reconcileTxn } = books;
  ...
  return (
    <PageShell title="Banking" subtitle="Reconcile statement to books. Transfers and CSV post journals. Live bank feeds are not connected.">
      <div className="flex flex-wrap gap-2">
        {([['recon', 'Reconcile'], ['move', 'Transfer / journal'], ['import', 'CSV import']] as const).map(([id, label]) => (
          <button
            key={id}
            type="button"
            onClick={() => setTab(id)}

...
export function extractMoneyAmount(text: string): ParsedMoneyAmount | null {
  ...
  // Only if nothing currency-like was found — still never take bare day-of-month or decoys.
  if (!hits.length) {
...
export function reconcileVisionAmount(
  ...
  // Vision amount not next to ₹, but OCR has a ₹ amount → trust OCR.
  if (!visionInRupee && textInRupee) return textAmt;
  if (!visionInRupee && textStrong) return textAmt;
  // Strong payment-verb parse beats a conflicting vision guess.

        ...
        try {
            ...
            new Thread(() -> {
                ...
                try {
                    ...
                    // Firebase Auth (skipNativeAuth) only needs the idToken. Access-token
                    // fetch often fails with Account reauth / BAD_AUTHENTICATION on devices
                    // even after a successful account pick — do not fail the whole sign-in.
                    Log.w(
                        FirebaseAuthenticationPlugin.TAG,
                        "Google access token unavailable; continuing with idToken only.",
                        exception
                    );
                    accessToken = null;
                }
